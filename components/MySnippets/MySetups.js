...
const MySetups = ({ selectedSubFolder, loadingMain, setLoadingMain }) => {
  ...
  const getMySnippets = async () => {
    ...
    try {
      ...
      if (!colEmpty) {
        ...
        const setups = snippetDocs.docs.map((doc) => ({
          id: doc.id,
          ...doc.data(),
        }));
  ...
  const fetchMore = async () => {
    ...
    try {
      ...
      if (!colEmpty) {
        ...
        setMySetups((prev) => [...prev, ...snippets]);
        setLoadingSub(false);
      } else {
        setIsEmpty(true);
        setLoadingSub(false);
        ...
        <>
          ...
          <div className="w-full">
            <div className="flex flex-col gap-4">
              {Object.keys(mySetups).length > 0 && (
                <>
                  {mySetups?.map((snippet) => (
                    <Snippet
                      key={snippet.id}
                  ...
                  {loadingSub ? (
                    ...
                    <>
                      {!mySetups?.length > 0 && (
                        <div className="flex justify-center mt-10">
                          <Text b size={13} transform="uppercase">
                            Du har ingen Setups i denne mappe! 😔
                          </Text>
                        </div>
                      )}
                    </>
